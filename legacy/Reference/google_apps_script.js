/**
 * Tool Crib Automation - Google Apps Script Backend (Updated V4.1 - Multilingual & Borrow Photo)
 * 
 * INSTRUCTIONS:
...
function doPost(e) {
  try {
    ...
    switch (action) {
      ...
      case "borrowTool": result = borrowTool(data); break;
      case "returnTool": result = returnTool(data); break;
  ...
  try {
    ...
    if (data.imageBase64) {
      try {
        ...
        const blob = Utilities.newBlob(Utilities.base64Decode(data.imageBase64.split(',')[1] || data.imageBase64), MimeType.JPEG, data.imageName || `borrow_${data.toolId}.jpg`);
    ...
    // Col 13 = Borrow Image, Col 14 = Return Image
...
function returnTool(data) {
  ...
  try {
    ...
    if (transRow == -1) return { error: "No active borrow found" };

    const invData = inventorySheet.getDataRange().getValues();
    for (let i = 1; i < invData.length; i++) {
      if (invData[i][0] == data.toolId && invData[i][3] !== "จำนวนมาก") {
    ...
    if (data.imageBase64) {
      try {
        ...
        const blob = Utilities.newBlob(Utilities.base64Decode(data.imageBase64.split(',')[1] || data.imageBase64), MimeType.JPEG, data.imageName || `return_${data.toolId}.jpg`);
        const file = folder.createFile(blob);
        file.setSharing(DriveApp.Access.ANYONE_WITH_LINK, DriveApp.Permission.VIEW);
        // Correct: Column 14 for Return Image
        transSheet.getRange(transRow, 14).setValue(file.getUrl());
      } catch (e) { transSheet.getRange(transRow, 14).setValue("Upload Error: " + e.toString()); }
    }
    return { success: true };
  } finally { lock.releaseLock(); }
}
...
function getTransactions() {
  ...
  for (let i = 1; i < data.length; i++) {
    ...
    transactions.push({
      ...
      borrowImage: row[12] || "", // Col 13
      returnImage: row[13] || ""  // Col 14
    });
  }
