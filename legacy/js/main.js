...
document.getElementById('dropdownHelpBtnGuest')?.addEventListener('click', openHelpModal);
document.getElementById('dropdownHelpBtnUser')?.addEventListener('click', openHelpModal);
document.getElementById('regHelpBtn')?.addEventListener('click', openHelpModal);
...
function createToolCard(tool) {
    ...
    if (tool.imageUrl && tool.imageUrl.trim() !== '') {
        imageContent = `<img src="${tool.imageUrl}" alt="${tool.toolName}" style="width:100%; height:100%; object-fit:cover;" loading="lazy">`;
    }
        ...
        if (item.imageBase64) {
            imgPreviewHTML = `
                <div class="relative w-full h-32 rounded-lg overflow-hidden mt-2 border border-gray-200">
                    <img src="${item.imageBase64}" class="w-full h-full object-cover" alt="Uploaded condition preview for ${tool.toolName}">
                </div>
            `;
        }
        ...
        itemEl.innerHTML = `
             <div class="flex items-start gap-4">
                <div class="w-16 h-16 bg-gray-200 rounded-lg shrink-0 overflow-hidden">
                    ${tool.imageUrl ? `<img src="${tool.imageUrl}" class="w-full h-full object-cover" alt="${tool.toolName}" loading="lazy">` : ''}
                </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 mt-2">
                <div class="relative">
                    <select id="ret-cond-${index}" class="h-10 w-full pl-3 pr-10 rounded-lg border-gray-300 text-sm font-medium bg-gray-50 text-gray-900 dark:bg-[#19141e] dark:text-white dark:border-gray-600 appearance-none cursor-pointer focus:ring-2 focus:ring-primary focus:border-transparent transition-all" onchange="returnCart[${index}].condition = this.value; saveCartToLocalStorage()">
                ...
                <input type="text" placeholder="Notes..." class="h-10 rounded-lg border-gray-300 text-sm bg-gray-50 text-gray-900 dark:bg-[#19141e] dark:text-white dark:placeholder-gray-400 dark:border-gray-600 px-3 font-medium" value="${item.notes}" onchange="returnCart[${index}].notes = this.value; saveCartToLocalStorage()">
            </div>
...
function renderCartItems() {
    ...
    cart.forEach((item, index) => {
        ...
        if (tool.imageUrl) {
            thumb = `<div class="w-16 h-16 bg-gray-200 rounded-lg shrink-0 overflow-hidden"><img src="${tool.imageUrl}" class="w-full h-full object-cover" alt="${tool.toolName}" loading="lazy"></div>`;
        }
        ...
        if (item.imageBase64) {
            imgPreviewHTML = `
                <div class="relative w-16 h-16 rounded-lg overflow-hidden border border-gray-300 mt-2 sm:mt-0">
                    <img src="${item.imageBase64}" class="w-full h-full object-cover" alt="Uploaded checkout preview for ${tool.toolName}">
                </div>
            `;
        }
...
async function handleCartSubmit() {
    ...
    if (returnDate < todayStr) {
        showMessage("Return date cannot be in the past", "error");
        return;
    }
...
function updateReturnModeUI() {
    ...
    if (!returnFab) {
        ...
        returnFab.className = 'fixed bottom-6 right-24 w-auto h-14 bg-red-600 text-white rounded-full shadow-lg flex items-center justify-center px-6 gap-2 hover:bg-red-700 hover:scale-105 active:scale-95 transition-all z-40 hidden';
        returnFab.innerHTML = `
            <span class="material-symbols-outlined">keyboard_return</span>
...
function saveCartToLocalStorage() {
    ...
    const serializedReturnCart = returnCart.map(item => ({
        tool: item.tool,
        condition: item.condition,
        notes: item.notes,
        imageBase64: item.imageBase64
    }));
    localStorage.setItem('bctl_cart_borrow', JSON.stringify(serializedCart));
    localStorage.setItem('bctl_cart_return', JSON.stringify(serializedReturnCart));
}

function loadCartFromLocalStorage() {
    try {
        const storedCart = localStorage.getItem('bctl_cart_borrow');
        const storedReturnCart = localStorage.getItem('bctl_cart_return');
        if (storedCart) {
        ...
        if (storedReturnCart) {
            returnCart = JSON.parse(storedReturnCart).map(item => ({
                tool: item.tool,
                condition: item.condition || 'สภาพดี',
                notes: item.notes || '',
                imageFile: null,
                imageBase64: item.imageBase64
            }));
