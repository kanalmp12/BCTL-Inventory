 ...
 * Show Set PIN Modal
 */
window.showSetPinModal = function() {
 ...
 * Hide Set PIN Modal
 */
window.hideSetPinModal = function() {
    const modal = document.getElementById('setPinModal');
    if (modal) {
...
// Event Listeners for Set PIN Modal
document.getElementById('closePinModal')?.addEventListener('click', window.hideSetPinModal);
document.getElementById('setPinForm')?.addEventListener('submit', async (e) => {
    ...
    if (newPin !== confirmPin) {
        showMessage('PIN codes do not match', 'error');
        return;
    }
    ...
    try {
        ...
        if (typeof updateUserPin === 'function') {
            const result = await updateUserPin(userId, newPin);
            if (result.success) {
                ...
                showMessage('PIN code updated successfully', 'success');
                window.hideSetPinModal();
            } else {
                 throw new Error(result.error || 'Failed to update PIN');
            }
        } else {
            console.error('updateUserPin function not found');
            // Fallback for mock if function missing (shouldn't happen)
            const currentUser = getUserInfo();
            if (currentUser) {
                currentUser.pin = newPin;
                saveUserInfo(currentUser);
            }
            showMessage('PIN code updated (Local Only)', 'success');
            window.hideSetPinModal();
        }
    } catch (error) {
        console.error('Error setting PIN:', error);
        showMessage('Failed to set PIN. Please try again.', 'error');
    } finally {
        showLoading(false);
    }
});